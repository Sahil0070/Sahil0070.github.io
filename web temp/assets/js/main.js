(function ($) {
  ...
  $('#mxbtn').click(function () {
    ...
    // console.log(subval);
    if (minval && maxval) {
      location.assign("shop-grid-left-sidebar.php?min=" + minval + "&max=" + maxval + "&subcat=" + subval);
  ...
  $(".delet-btn").on("click", function () {
    ...
    $.ajax({
      url: "/web temp/crud.php",
      type: "GET",
      dataType: "json",
      data: { cartid: cartId, action: "deletecart" },
      beforeSend: function () {
      ...
      success: function (subcategory) {
        ...
        // window.location.href('localhost/web temp/cart.php?');
        // location.refresh();
        // history.go(0);
  ...
  $('#deletebtn').on("click", function () {
    ...
    $.ajax({
      ...
      type: "GET",
      dataType: "json",
      data: { listid: listId, action: "deletelist" },
      beforeSend: function () {
  ...
  $(".qtybutton").on("click", function () {
    ...
    var total = $button.data('price');
    var cartId = $button.data('id');
    //var staticval ="#totalprice";
    if ($button.text() === "+") {
      ...
      $.ajax({
        ...
        type: "GET",
        dataType: "json",
        data: { cartid: cartId, qty: newVal, action: "updatecart" },
        beforeSend: function () {
        ...
        success: function (subcategory) {
          ...
          // location.assign("cart.php")
          $("#totalprice".concat(cartId)).text("Rs. ".concat(total * newVal));
          addtotalamount();
  ...
  function addtotalamount() {
    $.ajax({
      ...
      success: function (subcategory) {
        ...
        if (subcategory) {
          // $.each(subcategory.total, function (index, subcate) {
          $('#totalamount').html("Rs. ".concat(subcategory));
          $.session.set('totalamount', subcategory);
          subcategory = subcategory + 50;
          $('#totalamt').html("Rs. ".concat(subcategory));
