...
class SubCategory extends Database
{
    ...
    */
    ...
    {
        $ctable ='category';
        $sql = "SElECT category_Name,sub_Category_Name,sub_Category_Id FROM ${ctable},{$this->tableName} WHERE ${ctable}.category_Id = {$this->tableName}.category_Id ORDER BY sub_Category_Id  DESC LIMIT {$start},{$limit}";
        $stmt = $this->conn->prepare($sql);
        $stmt->execute();
        if($stmt->rowCount() > 0){
    ...
    */
    ...
    {
        $sql = "SELECT * FROM {$this->tableName} WHERE sub_Category_Name LIKE :search ORDER BY sub_Category_Id DESC LIMIT {$start}{$limit}";
        $stmt = $this->conn->prepare($sql);
