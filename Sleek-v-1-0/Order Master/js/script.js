...
function getplayerrow(Order) {
    ...
    if (Order) {
        ...
        <td class="align-middle">${Order.currentdate}</td>
      
        <td class="align-middle">      
        <a href="#" class="btn btn-warning mr-3 edituser"  data-target="#userModal" title="Edit"  data-id="${Order.order_Id}"><i class="fa fa-pencil-square-o fa-lg"></i></a>
        <a href="#" class="btn btn-danger deleteuser" data-userid="14" title="Delete"  data-id="${Order.order_Id}"><i class="fa fa-trash-o fa-lg"></i></a>
        </td>
        </tr>`;
...
function getplayers() {
    ...
    $.ajax({
        url: "/Sleek-v-1-0/Order Master/ajax.php",
        type: "GET",
        dataType: "json",
        data: { page: pageno, action: "getordermaster" },
        beforeSend: function () {
            $('#overlay').fadeIn();
        },
        success: function (rows) {
            if (rows.Order) {
                ...
                $.each(rows.Order, function (index, order) {
                    Orderlist += getplayerrow(order);
                });
                $('#userstable tbody').html(Orderlist);

                let totalordermaster = rows.count;
                let totalpages = Math.ceil(parseInt(totalordermaster) / 4);
        ...
        if (searchText.length > 1) {
            $.ajax({
                ...
                success: function (UserName) {
                    if (UserName) {
                        var userlist = "";
                        $.each(UserName, function (index, user) {
