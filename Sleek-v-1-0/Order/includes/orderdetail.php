...
class SubCategory extends Database
{
    ...
    */
    public function getRows($start = 0, $limit = 4)
    {
        ...
        $Omtable = 'order_master';
        $sql = "SELECT {$this->tableName}.order_Detail_Id,{$Omtable}.order_Id,{$ptable}.product_Name, {$this->tableName}.quantity, {$this->tableName}.product_Price FROM ${ptable},{$this->tableName},{$Omtable} WHERE {$ptable}.product_Id = {$this->tableName}.product_Id AND {$Omtable}.order_Id = {$this->tableName}.order_Id ORDER BY order_Detail_Id  DESC LIMIT {$start},{$limit}";
        $stmt = $this->conn->prepare($sql);
        $stmt->execute();
        if ($stmt->rowCount() > 0) {
    ...
    */
    ...
    public function searchPlayer($searchText, $start = 0, $limit = 4)
    {
        $ptable = 'product_description';
        $Omtable = 'order_master';
        $sql = "SELECT {$this->tableName}.order_Detail_Id,{$Omtable}.order_Id,{$ptable}.product_Name, {$this->tableName}.quantity, {$this->tableName}.product_Price FROM ${ptable},{$this->tableName},{$Omtable} WHERE {$ptable}.product_Id = {$this->tableName}.product_Id AND {$Omtable}.order_Id = {$this->tableName}.order_Id AND product_Name LIKE :search ORDER BY order_Detail_Id DESC LIMIT {$start}{$limit}";
        $stmt = $this->conn->prepare($sql);
